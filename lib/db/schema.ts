...
export const documentChunk = pgTable('DocumentChunk', {
  ...
  pineconeId: varchar('pineconeId', { length: 100 }).notNull().unique(),
...
export const chatKnowledgeBase = pgTable('ChatKnowledgeBase', {
  chatId: uuid('chatId')
    .notNull()
    .references(() => chat.id),
  knowledgeBaseId: uuid('knowledgeBaseId')
  ...
  pk: primaryKey({ columns: [table.chatId, table.knowledgeBaseId] }),
}));

export type ChatKnowledgeBase = InferSelectModel<typeof chatKnowledgeBase>;
